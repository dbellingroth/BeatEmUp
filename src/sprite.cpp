...
void Sprite::flipSurface(SDL_Surface* src, SDL_Surface* desk, Vec2i offset,
		Vec2i size) {
...
std::list<Sprite> Sprite::getSubImages(const std::string image_path, int nx,
		int ny) {
	...
	for (int yp = 0; yp < ny; yp++) {
		for (int xp = 0; xp < nx; xp++) {
			SDL_Surface* sub_surface = SDL_CreateRGBSurface(SDL_SWSURFACE,
